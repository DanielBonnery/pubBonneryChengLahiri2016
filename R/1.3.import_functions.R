#creation of function that imports the tables
Importecsv<-function(tables,rep){
for (i in tables){
	x<-         read.csv(paste(i,".csv",sep=''));
	names(x)<-toupper(names(x))
    assign(i,x,envir=parent.frame())}
return(lapply(tables,get))}

Importe<-function(tables,extension,rep){
  if(extension=="csv") {impo<-read.csv}
   if(extension=="xpt"){impo<-sasxport.get}
return(lapply(tables,function(i){impo(paste(rep,"/",i,".",extension,sep=''))}))
}

#creation of function that imports the tables
Importesas<-function(dir,list.tables=NULL){
	if(is.null(list.tables)){list.tables<-sub("^([^.]*).*", "\\1",list.files(path=dir,pattern="\\.sas7bdat"))}
	for(f in list.tables){assign(f,sas.get(lib=dir,mem=f,formats=FALSE),envir=.GlobalEnv)}
	return(lapply(list.tables,get))}
