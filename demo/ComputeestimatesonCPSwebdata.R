...
Estimates_AK_e_web<-AK(list.tables = list.tablesweb,w = "pwsswgt",id = c("hrhhid","pulineno"),groupvar = "hrmis",
                       groups_1 =c("2","3","4","5","7","8"),groups_0=paste0(c(1:3,5:7),""),list.y="employed",A=ACPSemployed(),K=KCPSemployed())
Estimates_AK_u_web<-AK(list.tables = list.tablesweb,w = "pwsswgt",id = c("hrhhid","pulineno"),groupvar = "hrmis",
                       groups_1 =c("2","3","4","5","7","8"),groups_0=paste0(c(1:3,5:7),""),list.y="unemployed",A=ACPSunemployed(),K=KCPSunemployed())
...
cbind(Estimates_AK_web[1,,1:2],Estimates_AK_u_web,Estimates_AK_e_web)
...
dimnames(Estimates_MR_webA)[[1]]<-paste0("MR",names(Estimates_MR_webA))
dimnames(Estimates_MR_webA)[[2]]<-allmonths

#6. Put all estimates in the same dataframe
Estimates_all_web<-abind::abind(Estimates_MR_webA,Estimates_AK_web,Direct=array(Estimates_Direct_web,c(1,dim(Estimates_Direct_web))),along=1)
...
figure2.a<-ggplot(data=Estimates_all_rel_webD[Estimates_all_rel_webD$Variable=="r"&
                                 is.element(Estimates_all_rel_webD$Estimator,c("MR0","MR1","MR0.75","AK3-0.3,0.4,0.4,0.7")),],
...
figure2.b<-ggplot(data=Estimates_all_change_rel_webD[Estimates_all_change_rel_webD$Variable=="r"&
                                                is.element(Estimates_all_change_rel_webD$Estimator,c("MR0","MR1","MR0.75","AK3-0.3,0.4,0.4,0.7")),],
                  aes(x=Month,y=value)) + geom_line(aes(color=Estimator))+ scale_y_continuous("Unemployment rate") +
  scale_x_date(date_breaks = "1 year", date_minor_breaks = "1 month", date_labels="%Y")+xlab("Time") 


#setwd("~Dropbox/CensusBAE/R/Computation of FR/packages/pubBonneryChengLahiri2016")
